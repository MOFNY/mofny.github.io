...
$.each(buttonArray, function(i, item) {
	$(this).click(function(){
		...
		$(hideGroupArray[i-1]).slideToggle(slideToggleDuration, function(){
			...
			//setTimeout(function() {$(buttonArray[i]).css({opacity: "1", width: "10%"})},800);
			if ($(groupArray[i-1]).height() > 0)
			{
				...
				//closeBar.show().addClass("floatCloseBar");
				...
				//$(allButtonArray[j]).hide().appendTo(closeBar).show(100);
